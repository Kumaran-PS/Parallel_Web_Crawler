...
//    return topCounts;
    ...
    WordCountComparator comparator = new WordCountComparator();// creating a object reference WordCountComparator
    return wordCounts
            .entrySet()  // returns the key value entry set
            .stream()       //entry set is passed as a stream
            .filter(Objects::nonNull)  // to filter the objects which are  not null(elmination null objects)
            .sorted(comparator)     // to sort the entryset by using comparator method
            .limit(Math.min(popularWordCount , wordCounts.size()))
            // below to collect the final entryset
            .collect(Collectors.toMap(Map.Entry::getKey , Map.Entry::getValue, (key , val) ->key , LinkedHashMap::new ));

// to test -  mvn test -Dtest=WordCountsTest
